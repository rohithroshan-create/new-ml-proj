...
# ===== TAB 1: DELIVERY RISK & DELAY (FIXED) =====
with tab1:
        ...
        try:
            ...
            if missing_clf_cols or missing_reg_cols:
                ...
                with col1:
                    ...
                    # === FIX for KeyError: 'index' ===
                    # Explicitly name the columns to avoid ambiguity
                    risk_counts = df_results['Predicted_Late_Risk'].value_counts().reset_index()
                    risk_counts.columns = ['Risk_Label', 'Count'] # Name them explicitly
                    risk_counts['Risk_Label'] = risk_counts['Risk_Label'].map({0: 'On Time', 1: 'Late Risk'})
                    
                    fig_pie = px.pie(risk_counts, 
                                     names='Risk_Label',  # Use the new explicit column name
                                     values='Count',    # Use the new explicit column name
                                     title="Risk Prediction Overview",
                                     color='Risk_Label', color_discrete_map={'On Time':'green', 'Late Risk':'red'})
                ...
                with st.expander("View full prediction data"):
                    # === FIX for KeyError: 'Order Id' ===
                    # Define the core columns we know exist
                    display_cols = ['Predicted_Late_Risk', 'Predicted_Late_Probability', 'Predicted_Shipping_Days', 'Days for shipment (scheduled)']
                    
                    # Check if 'Order Id' exists in the dataframe before adding it
                    if 'Order Id' in df_results.columns:
                        display_cols.insert(0, 'Order Id') # Add it to the front *only* if it exists
                    
                    # Get the rest of the classifier columns, avoiding duplicates
                    other_cols = [col for col in cols_clf if col not in display_cols]
                    
                    # Display the dataframe with the safe list of columns
                    st.dataframe(df_results[display_cols + other_cols].head(100), use_container_width=True)
...
with tab3:
    ...
    if uploaded_file_3 and churn_model and supplier_model:
        try:
            ...
            try:
                # This assumes churn_model is a pipeline that handles preprocessing.
                # You MUST upload data with the correct columns.
            ...
            try:
                # This assumes supplier_model is a pipeline
                ...
                col1, col2 = st.columns(2)
                # Assuming a 'SupplierID' column, fallback to count
                supplier_count = df['SupplierID'].nunique() if 'SupplierID' in df.columns else len(df)
                col1.metric("Total Records", f"{supplier_count}")
                col2.metric("Avg. Reliability Score", f"{avg_score:.2f}")
...
# ===== TAB 4: BLOCKCHAIN CHATBOT (FIXED) =====
with tab4:
    ...
    # --- Gemini API Setup ---
    # === FIX for NameError: name 'st' is not defined ===
    # Paste your API key here as a string.
    GEMINI_API_KEY = "PASTE_YOUR_GEMINI_API_KEY_HERE"

    if GEMINI_API_KEY == "PASTE_YOUR_GEMINI_API_KEY_HERE":
        st.error("Please add your Gemini API Key to the code (line 307) to use the chatbot.", icon="🚨")
        model = None
    else:
        try:
