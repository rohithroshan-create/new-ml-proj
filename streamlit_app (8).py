        ...
        try:
            ...
            if missing_clf_cols or missing_reg_cols:
                ...
                with col1:
                    ...
                    risk_counts.columns = ['Risk_Label', 'Count'] # Fix potential column naming issue
                ...
                with st.expander("View full prediction data"):
                    # Check if 'Order Id' exists before trying to display it
                    ...
                    if 'Order Id' in df_results.columns:
                        display_cols.insert(0, 'Order Id')
                    
                    st.dataframe(df_results[display_cols + [col for col in cols_clf if col not in display_cols]].head(100), use_container_width=True)
...
with tab4:
    ...
    # --- Gemini API Setup ---
    # !!! FIX IS HERE !!!
    # Paste your API key here as a string. This avoids the st.secrets error.
    GEMINI_API_KEY = "PASTE_YOUR_GEMINI_API_KEY_HERE"

    if GEMINI_API_KEY == "PASTE_YOUR_GEMINI_API_KEY_HERE":
        st.error("Please add your Gemini API Key to the code (line 297) to use the chatbot.", icon="🚨")
        model = None
    else:
        try:
